...
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY as string, { // Consider using a better way to handle env vars
...
export async function POST(req: Request) {
  try {
    ...
    if (cart && Array.isArray(cart) && cart.length > 0) {
      ...
      metadata.cartItems = JSON.stringify(cart.map(item => ({ // Stringify cart items
